...
struct tcp_stream *
tcp_find_host_by_ip_port(uint32_t sip, uint32_t dip, uint16_t sport, uint16_t dport)
{
    struct tcp_streams *tcp_list_inst = tcp_list_instance();
    for (struct tcp_stream *streami = tcp_list_inst->stream_head; streami != NULL; streami = streami->next)
    {
        if (streami->sip == sip &&
            streami->dip == dip &&
            streami->sport == sport &&
...
{
    struct tcp_stream *new_stream = rte_malloc("tcp_stream", sizeof(struct tcp_stream), 0);
    if (new_stream == NULL)
        return NULL;
    new_stream->sip = sip;
    new_stream->dip = dip;
...
void process_tcp(struct rte_mbuf *m)
{
    ...
    if ((tcpcksum) != mycksum)
    {
        RTE_LOG(INFO, APP, "tcphdr cksum = %x , my cksum = %x \n", tcphdr->cksum, mycksum);
        return;
    }

    struct tcp_stream *tcp_s = tcp_find_host_by_ip_port(iphdr->src_addr, iphdr->dst_addr, tcphdr->src_port, tcphdr->dst_port);
    if (tcp_s == NULL)
    {
        tcp_s = tcp_stream_create(iphdr->src_addr, iphdr->dst_addr, tcphdr->src_port, tcphdr->dst_port);
        if (tcp_s == NULL)
            return;
    }
    switch (tcp_s->status)
...
void tcp_handle_listen(struct tcp_stream *tcp_s, struct rte_tcp_hdr *tcphdr)
{
    ...
    if ((tcp_flgs & RTE_TCP_SYN_FLAG) != 0)
    {
        ...
        struct tcp_frame *tcp_pd = rte_malloc("tcp_frame", sizeof(struct tcp_frame), 0);
...
struct rte_mbuf *encode_tcp(struct tcp_frame *tcp_pd, uint32_t sip, uint32_t dip)
{
    ...
    fill_ip_hdr(m, total_len - sizeof(struct rte_ether_hdr),
                dip, sip, IPPROTO_TCP);

    struct rte_ipv4_hdr *iphdr = rte_pktmbuf_mtod_offset(m, struct rte_ipv4_hdr *, sizeof(struct rte_ether_hdr));
    struct rte_tcp_hdr *tcphdr = rte_pktmbuf_mtod_offset(m, struct rte_tcp_hdr *, sizeof(struct rte_ether_hdr) + sizeof(struct rte_ipv4_hdr));
    ...
    if (tcp_pd->data != NULL)
    {
        uint8_t *payload = (uint8_t *)(tcphdr + 1) + tcp_pd->optlen * sizeof(uint32_t);
...
void tcp_out(void)
{
    ...
    for (struct tcp_stream *streami = tcp_list->stream_head; streami != NULL; streami = streami->next)
    {
        int ret;
