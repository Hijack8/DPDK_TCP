...
uint8_t broadcast_addr[RTE_ETHER_ADDR_LEN] = {0xFF, 0xFF, 0xFF,
                                              0xFF, 0xFF, 0xFF};
...
void app_init_rings() {
  eth_ring = (struct ring_buffer *)rte_malloc("eth_ring",
                                              sizeof(struct ring_buffer), 0);
  proto_ring = (struct ring_buffer *)rte_malloc("proto_ring",
                                                sizeof(struct ring_buffer), 0);
  tcp_ring = (struct ring_buffer *)rte_malloc("tcp_ring",
                                              sizeof(struct ring_buffer), 0);
  eth_ring->in = rte_ring_create("eth_ring_in", RING_SIZE, rte_socket_id(),
                                 RING_F_SP_ENQ | RING_F_SC_DEQ);
  eth_ring->out = rte_ring_create("eth_ring_out", RING_SIZE, rte_socket_id(),
                                  RING_F_SP_ENQ | RING_F_SC_DEQ);
  proto_ring->in = rte_ring_create("proto_ring_in", RING_SIZE, rte_socket_id(),
                                   RING_F_SP_ENQ | RING_F_SC_DEQ);
  proto_ring->out =
      rte_ring_create("proto_ring_out", RING_SIZE, rte_socket_id(),
                      RING_F_SP_ENQ | RING_F_SC_DEQ);
  tcp_ring->in = rte_ring_create("tcp_ring_in", RING_SIZE, rte_socket_id(),
                                 RING_F_SP_ENQ | RING_F_SC_DEQ);
  tcp_ring->out = rte_ring_create("tcp_ring_out", RING_SIZE, rte_socket_id(),
                                  RING_F_SP_ENQ | RING_F_SC_DEQ);
  if (eth_ring->in == NULL || eth_ring->out == NULL || proto_ring->in == NULL ||
      proto_ring->out == NULL || tcp_ring->in == NULL ||
      tcp_ring->out == NULL) {
...
int app_main_loop_nic() {
  ...
  while (1) {
    ...
    if (n_bufs != 0) {
      int ret =
          rte_ring_enqueue_burst(eth_ring->in, (void **)bufs, n_bufs, NULL);
      if (ret > 0 && ret < n_bufs) {
        rte_memcpy(bufs, bufs + ret,
                   (n_bufs - ret) * sizeof(struct rte_mbuf *));
      }
      n_bufs -= ret;
    }

    int nb_rd_tx =
        rte_ring_dequeue_burst(eth_ring->out, (void **)bufs_tx, 8, NULL);
    n_bufs_tx += nb_rd_tx;
    if (n_bufs_tx > 0) {
      int ret = rte_eth_tx_burst(port, 0, bufs_tx, n_bufs_tx);
      if (ret != n_bufs_tx) {
        rte_memcpy(bufs_tx, bufs_tx + ret,
                   (n_bufs_tx - ret) * sizeof(struct rte_mbuf *));
      }
      n_bufs_tx -= ret;
    }
  }
...
int fill_ether_hdr(struct rte_mbuf *m, uint8_t *src_mac, uint8_t *dst_mac,
                   uint16_t type) {
...
struct rte_mbuf *encode_arp(uint16_t arp_opcode, uint8_t *dst_mac,
  ...
  struct rte_ether_hdr *eth_hdr =
      rte_pktmbuf_mtod(arp_pkt, struct rte_ether_hdr *);
  rte_memcpy(eth_hdr->src_addr.addr_bytes, my_addr, RTE_ETHER_ADDR_LEN);
  // if (!strncmp((const char *)dst_mac, (const char *)broadcast_addr,
  // RTE_ETHER_ADDR_LEN))
  // {
  ...
  struct rte_arp_hdr *arp_hdr = rte_pktmbuf_mtod_offset(
      arp_pkt, struct rte_arp_hdr *, sizeof(struct rte_ether_hdr));
  arp_hdr->arp_hardware =
      rte_cpu_to_be_16(1); // 2 bytes need to change the order of bytes
...
static void arp_request_timer_cb(__attribute__((unused)) struct rte_timer *tim,
  ...
  struct rte_mbuf *arp_pkt =
      encode_arp(RTE_ARP_OP_REQUEST, broadcast_addr, gateway_ip, local_ip);
...
void process_arp(struct rte_mbuf *m) {
  struct rte_arp_hdr *arp_hdr = rte_pktmbuf_mtod_offset(
      m, struct rte_arp_hdr *, sizeof(struct rte_ether_hdr));
  ...
  if (arp_hdr->arp_opcode == rte_cpu_to_be_16(RTE_ARP_OP_REQUEST)) {
    ...
    if (index == -1) {
      int ret = append_arp_table(arp_hdr->arp_data.arp_sip,
...
int app_main_loop_eth() {
  ...
  while (1) {
    ...
    if (ret == 0) {
      ...
      if (index != -1) {
        int succ =
            fill_ether_hdr(m, my_addr, arp_table[index], RTE_ETHER_TYPE_IPV4);
...
void fill_ip_hdr(struct rte_mbuf *m, uint16_t len, uint32_t src_ip,
  ...
  // rte_cpu_to_be_16(sizeof(struct rte_ipv4_hdr) + sizeof(struct
  // new_icmp_hdr));
  ip_hdr->packet_id = 0;
...
struct rte_mbuf *encode_icmp(uint8_t *src_mac, uint8_t *dst_mac,
                             uint32_t src_ip, uint32_t dst_ip, uint16_t id,
                             uint16_t seqnb) {
  struct rte_mbuf *icmp_pkt = rte_pktmbuf_alloc(mbuf_pool);
  int total_size = sizeof(struct rte_ether_hdr) + sizeof(struct rte_ipv4_hdr) +
                   sizeof(struct new_icmp_hdr);
  icmp_pkt->data_len = total_size;
  icmp_pkt->pkt_len = total_size;
  fill_ether_hdr(icmp_pkt, src_mac, dst_mac, RTE_ETHER_TYPE_IPV4);
  fill_ip_hdr(icmp_pkt,
              sizeof(struct rte_ether_hdr) + sizeof(struct rte_ipv4_hdr),
              src_ip, dst_ip, IPPROTO_ICMP);

  struct new_icmp_hdr *icmp_hdr = rte_pktmbuf_mtod_offset(
      icmp_pkt, struct new_icmp_hdr *,
      sizeof(struct rte_ether_hdr) + sizeof(struct rte_ipv4_hdr));
  ...
  icmp_hdr->icmp_cksum =
      icmp_checksum((uint16_t *)icmp_hdr, sizeof(struct new_icmp_hdr));
  return icmp_pkt;
}
...
void process_icmp(struct rte_mbuf *m) {
  ...
  if (icmp_hdr->icmp_type == RTE_IP_ICMP_ECHO_REPLY) {
    ...
    struct rte_mbuf *m = encode_icmp(
        my_addr, eth_hdr->src_addr.addr_bytes, ip_hdr->dst_addr,
        ip_hdr->src_addr, icmp_hdr->icmp_ident, icmp_hdr->icmp_seq_nb);
    int ret;
    do {
...
void app_init() {
  mbuf_pool =
      rte_pktmbuf_pool_create("app_pool", (1 << 15) - 1, 256, 0,
